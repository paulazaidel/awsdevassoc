...
aws iam create-policy --policy-name "ADGUIOT_Lambda_Policy" --policy-document '{ "Version": "2012-10-17", "Statement": [ { "Sid": "VisualEditor0", "Effect": "Allow", "Action": [ "dynamodb:BatchGetItem", "dynamodb:PutItem", "dynamodb:DescribeTable", "dynamodb:DeleteItem", "dynamodb:GetItem", "dynamodb:Scan", "dynamodb:Query", "dynamodb:UpdateItem" ], "Resource": [ "arn:aws:dynamodb:us-west-1:'$acct':table/ADGUIOT/index/*", "arn:aws:dynamodb:us-west-1:'$acct':table/ADGUIOT" ] }, { "Sid": "VisualEditor1", "Effect": "Allow", "Action": [ "states:SendTaskSuccess", "logs:CreateLogStream", "states:SendTaskFailure", "logs:CreateLogGroup", "logs:PutLogEvents" ], "Resource": "*" } ] }'
echo
